...
Installing Coplay MCP for Claude Code: `claude mcp add --scope user --transport stdio coplay-mcp --env MCP_TOOL_TIMEOUT=720000 -- uvx --python ">=3.11" coplay-mcp-server@latest`

Or add this config to your favorite MCP Host (e.g. Cursor or Cline):
```json
{
  "mcpServers": {
    "coplay-mcp": {
...
Note: We set the timeout to 720s (12 minutes) because using the coplay_task tool can run for quite a while in Unity depending on the task. If you don't plan to use the coplay_task tool, you can set the timeout to a more reasonable value such as 60s.
