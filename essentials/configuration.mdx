...
- **StepByStep** – Pauses after every action and waits for your feedback before continuing. Use this for tasks that require precise placement or manual oversight; slower but more accurate.
- **OneShotSceneGen** – Generates an entire scene in a single prompt. Primarily used for large scene generation or when providing your own 3D models. This mode is being deprecated since Normal mode now handles most scene generation tasks.
- **Experimental** – Includes cutting‑edge features such as batch tool calls that speed up execution. This mode is intended for testing and may be less stable.
